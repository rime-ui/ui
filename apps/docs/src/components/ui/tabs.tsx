...
>((props, ref) => (
    <Unstyled.TabsList
        ref={ref}
        {...props}
        className={cn('text-fd-secondary-foreground not-prose flex gap-3.5 overflow-x-auto px-4', props.className)}
    />
))
...
>((props, ref) => (
    <Unstyled.TabsTrigger
        ...
        {...props}
        className={cn(
            'text-fd-muted-foreground hover:text-fd-accent-foreground cursor-pointer data-[state=active]:border-fd-primary data-[state=active]:text-fd-primary inline-flex items-center gap-2 border-b border-transparent py-2 text-sm font-medium whitespace-nowrap transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:size-4',
            props.className,
        )}
    />
))
...
export function Tabs({
    ...
    return (
        <Unstyled.Tabs
            ref={ref}
            className={cn('bg-fd-secondary my-4 flex flex-col overflow-hidden rounded-xl border', className)}
            value={value}
